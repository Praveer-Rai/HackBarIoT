...
Matrix::Matrix(uint8_t data, uint8_t clock, uint8_t load, uint8_t screensX /* = 1 */, uint8_t screensY /* = 1 */)
{
  ...
  _screens = screensX * screensY;
  _buffer = (uint8_t*)calloc(_screens, 64);
  _maximumX = (screensX * 8);
  _maximumY = (screensY * 8);
...
void Matrix::buffer(uint8_t x, uint8_t y, uint8_t value)
{
  ...
  if (x > _maximumX || y > _maximumY) return;
...
void Matrix::write(uint8_t lines, uint8_t oktets, uint8_t img[])
{
	for (uint8_t i = 0; i < lines; i++) {
		for (uint8_t j = 0; j < oktets; j++) {
			for (uint8_t k = 0; k < 8; k++) {
				buffer(k + 8 * (oktets - j - 1), i, (img[oktets - j - 1 + oktets * i] >> 7-k) & 0x01);
				//Serial.print((img[oktets - j - 1 + oktets * i] >> k) & 0x01);
				//Serial.print(" ");
